...
/// Borrowed from https://github.com/swiftlang/swift-tools-support-core/Sources/TSCBasic/HashAlgorithms.swift
...
protocol HashAlgorithm: Sendable {
...
extension HashAlgorithm {
    func hash(_ string: String) -> ByteString {
...
struct InternalSHA256: HashAlgorithm {
    ...
    private func process(_ block: ArraySlice<UInt8>, hash: inout [UInt32]) {
        ...
        for t in 0..<W.count {
            ...
            default:
                let σ1 =
                    W[t - 2].rotateRight(by: 17) ^ W[t - 2].rotateRight(by: 19) ^ (W[t - 2] >> 10)
                let σ0 =
                    W[t - 15].rotateRight(by: 7) ^ W[t - 15].rotateRight(by: 18) ^ (W[t - 15] >> 3)
...
struct CryptoKitSHA256: HashAlgorithm, Sendable {
